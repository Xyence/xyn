...
- `XYN_DB_TENANCY_MODE=shared_rds_db_per_sibling` (default when external mode is enabled)
- `XYN_DB_BOOTSTRAP_DATABASE_URL` (control-plane only; required for external tenant allocation)
...
- `XYN_DB_MODE=external`
  ...
  - requires `XYN_DB_TENANCY_MODE=shared_rds_db_per_sibling`
  - requires `XYN_DB_BOOTSTRAP_DATABASE_URL` (admin/bootstrap URL used only by control-plane allocation)
...
Security constraints for external DB mode:
- Never inject `XYN_DB_BOOTSTRAP_DATABASE_URL` into sibling runtimes.
- Runtime `DATABASE_URL` must use per-sibling scoped credentials generated by the allocator.
- Persist only non-secret allocation metadata (mode/tenancy/db name/user/host/port), never admin credentials.
